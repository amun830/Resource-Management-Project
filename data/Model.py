...
def create_pressure_model(f, t0, t1, dt, t_data, q_data, p0, p1,):
    def inner(t, a, b, p_init):
        timean, pressan, = solve_ode_pressure(f, t0, t1, dt, t_data, q_data, [a, b, p0, p1, p_init])
        return np.interp(t, timean, pressan)
    return inner

def create_cu_model(f, t0, t1, dt, t_sol, P_sol, p0, p1, a, b, M0):
    def inner(t, d, c_init, C_src):
        timean, cuan, = solve_ode_cu(f, t0, t1, dt, t_sol, P_sol,[a, b, d, p0, p1, c_init, C_src, M0])
        return np.interp(t, timean, cuan)
    return inner
...
if __name__ == "__main__":
    ...
    # Code using scipy.curvefit() to calibrate parameters
    if True:
        # Initialise model parameters 
        a = 1/(997*4184*0.0005) * 1.27          #Calibrate
        b = 1.5/(997*4184*0.0005) * 58          #Calibrate
        ...
        [a, b, p_init], _ = curve_fit(create_pressure_model(ode_pressure, t0, t1, dt, t_q_data, q_data, p0, p1), t_p_data, p_data)

        P_parameters = [a, b, p0, p1, p_init]

        t_fitted, P_fitted = plot_aquifer_pressure(t0, t1, dt, t_q_data, q_data, t_p_data, p_data, P_parameters)

        C_parameters = [a, b, d, p0, p1, c_init, C_src, M0]

        [d, c_init, C_src], _ = curve_fit(create_cu_model(ode_cu, t0, t1, dt, t_p_data, p_data, p0, p1, a, b, M0), t_cu_data, cu_data)

        C_parameters = [a, b, d, p0, p1, c_init, C_src, M0]

        plot_aquifer_cu(t0, t1, dt, t_p_data, p_data, t_cu_data, cu_data, C_parameters)   
    ...
    if True: 
        ...
        t_p_fcst, p_fcst = solve_ode_pressure(ode_pressure, t0, t1+time_into_future, dt, t_q_data, q_data, P_parameters)
        t_cu_fcst, cu_fcst = solve_ode_cu(ode_cu, t0, t1 + time_into_future, dt, t_p_data, p_data, C_parameters)

        f,host = plt.subplots(1,2)

        host[0].plot(t_p_fcst, p_fcst, c = 'r', label="Model Solution")
        host[1].plot(t_cu_fcst, cu_fcst, c = 'r', label="Model Solution")



        # Configure and show plot
        
        host[0].set_ylabel('Pressure (Pa)')
        host[0].set_xlabel('Time (Year)')
        host[0].set_title('Future Pressure Prediction')
        host[0].axvline(x=t1, c='g', ls="--", label='Current time')
        host[0].legend()

        host[1].set_ylabel('Cu (mass fraction')
        host[1].set_xlabel('Time (Year)')
        host[1].set_title('Future Cu Prediction')
        host[1].axvline(x=t1, c='g', ls="--", label='Current time')
        host[1].legend()
